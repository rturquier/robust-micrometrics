//	file: 
		local Namedo "voxA-mod103A-v01a-RepresentativeEstimatesPredictRMSE"
		
//	task: Predicts giving at each of the six budget decisions, using a SINGLE 
//			alpha, beta for ALL individuals (Representative).
//
//			Mimics for a single alpha, beta, the do-file "voxA-mod102A-v01a-IndvidEstimatesPredictRMSE.do"
//
//			The particular alpha, beta used would come from, say, estimation of a representative utility model (e.g., .594, .021).
//
...
* *** *** *** *** *** *** *** *** *** *** *** *** *** *** *	
*
* Select the alpha, beta estimates -- HOMOGENEOUS
*
*	Note: The conditional if will put "missing"s in for the participants 
*			who were at the corners, and who therefore did not figure into
*			the predictions for the heterogensous alpha, beta.
*

	local a = .569	// n = 78
	local b = .026

	generate alpha = `a'	if !missing(alpha001)
	generate  beta = `b'	if !missing(alpha001)
...
assert missing(alpha) if  missing(beta)		// Make sure if alpha is missing, beta is also missing (and vice-versa).
assert  missing(beta) if missing(alpha)
...
foreach x in 04 10 28 34 {
	local ++i
		...
		note		h`x'hat : Predictions based on alpha = `a'    beta = `a'   HOMOGENEOUS \ /*
								*/ Missing means that the heterogeneous (yes, the heterogeneous) alpha, beta estimates themselves were missing \ `tag'
...
note		h04_y46hat : Predictions based on alpha = `a'    beta = `a'   HOMOGENEOUS \ /*
								*/ Missing means that the heterogeneous (yes, the heterogeneous) alpha, beta estimates themselves were missing \ `tag'
...
note		h28_y46hat : Predictions based on alpha = `a'    beta = `a'   HOMOGENEOUS \ /*
								*/ Missing means that the heterogeneous (yes, the heterogeneous) alpha, beta estimates themselves were missing \ `tag'
